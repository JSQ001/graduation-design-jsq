...
class Importer extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      transactionID: null,
      errorColumns: [
        {title: "行号", dataIndex: 'index', width: '13%'},
  ...
  render() {
    ...
    return (
      <Modal className="importer"
             ...
             okText={tabKey === 'UPDATE' ? "导入" : "完 成"}
             cancelText={formatMessage({id: "common.cancel"}/*取消*/)}>
        <Tabs defaultActiveKey="UPDATE" activeKey={tabKey} onChange={(key) => {this.setState({ tabKey: key })}}>
          ...
          <TabPane tab="导入结果" key="SUCCESS" disabled={tabKey === 'UPDATE'}>
            <div>导入成功: {result.successEntities} || 0</div>
            <div>导入失败: {result.failureEntities} || 0
              {result.failureEntities ? (
                <span style={{fontSize:12,marginLeft:10}}>（请修改相应数据后，重新导入）</span>
              ) : ''}
            </div>
            {result.failureEntities > 10 ?
              <div style={{marginTop:10}}>
                <Icon type="exclamation-circle-o" style={{marginRight:5, color:'red'}}/>
                导入失败超过10条，请下载错误信息查看详情
              </div> : ''}
            {result.failureEntities ? (
              <div>
...
Importer.defaultProps = {
  title: '导入',
  fileName: '导入文件',
  listenUrl: `${config.baseUrl}/api/student/batch/transaction/logs`,
  onOk: () => {},
  afterClose: () => {},
};
