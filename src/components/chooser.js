...
class Chooser extends React.Component {
  ...
  render() {
    ...
    const { placeholder, disabled, selectorItem, type, listExtraParams, single, showNumber, newline } = this.props;
    return (
      <div className={`chooser ${showNumber && 'number-only'} ${disabled && 'chooser-disabled'} ${newline && 'newline'}`}>
        ...
        <Icon className="ant-select-selection__clear" type="close-circle" onClick={this.clear} style={value.length === 0 ? {opacity: 0} : {}}/>
