...
class BasicInfo extends React.Component{
    ...
    if (item.type.toLowerCase() === 'img' || item.type.toLowerCase() === 'image') {
      return (<Tooltip title={
        <img style={{width:200,height:200}}
             src={this.state.infoData[item.src]}/>
      }>
      ...
      return <Badge status={this.state.infoData[item.id] ? 'success' : 'error'}
                    text={this.state.infoData[item.id] ? this.props.intl.formatMessage({id: "common.status.enable"}/*启用*/) :
                      this.props.intl.formatMessage({id: "common.status.disable"}/*禁用*/)} />;
  ...
  render() {
    ...
    if(!this.props.isHideEditBtn){
      EditBtn = <a onClick={this.editInfo}>{this.props.intl.formatMessage({id: 'common.edit'}) /* 编辑 */}</a>
    }
    let domRender;
    if(cardShow) {
      domRender = (
...
BasicInfo.propTypes = {
  ...
  updateHandle: React.PropTypes.func,  //更新表单事件，只用来显示信息，不需要更新
  ...
  isHideEditBtn:React.PropTypes.bool,//是否隐藏编辑按钮
};
