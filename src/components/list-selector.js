...
class ListSelector extends React.Component {
  ...
  getList(){
    ...
    let reg = new RegExp(selectorItem.url);
    if( reg.test("?") ){
      //检测是否已经有参数了，如果有直接加&
  ...
  componentWillReceiveProps = (nextProps) => {
    ...
    if(nextProps.type !== this.state.type && !nextProps.selectorItem && nextProps.visible){
      ...
      if(nextProps.selectorItem && nextProps.visible){
      ...
      <Modal title={title} visible={visible} onCancel={onCancel} afterClose={afterClose} width={800} onOk={this.handleOk} className="list-selector">
        ...
        <div className="table-header">
          <div className="table-header-title">
            {this.props.intl.formatMessage({id: "common.total"}, {total: pagination.total || data.length})}{/* 共 total 条数据 */}
            &nbsp;<span>/</span>&nbsp;
