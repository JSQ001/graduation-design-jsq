...
class NewTeacher extends React.Component{
  ...
  handleSave = (e) =>{
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        console.log(values)
        values.role = [2];
        let method = !this.state.teacherDetail.id ? httpFetch.post(`${config.baseUrl}/api/user/insert`,values):
          httpFetch.put(`${config.baseUrl}/api/user/update`,{...values, id:this.state.teacherDetail.id});
        method.then((res)=>{
  ...
  render(){
    ...
    const { isEnabled, teacherDetail, dept,  } = this.state;
        ...
        <Form onSubmit={this.handleSave}>
          <FormItem {...formItemLayout} label="系部">
            {getFieldDecorator('deptId', {
              ...
              <Select disabled={!!teacherDetail.id}  className="input-disabled-color"
          ...
          <FormItem {...formItemLayout} label="姓名">
            {getFieldDecorator('nickName', {
              ...
              initialValue: teacherDetail.className
            })(
...
const WrappedNewTeacher = Form.create()(injectIntl(NewTeacher));

export default connect(mapStateToProps)(WrappedNewTeacher);
