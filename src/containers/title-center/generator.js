...
class Generator extends React.Component{
            ...
            <div className={editMode ? "form-item edit-item" : 'form-item'}>
              <div className="edit-operate">
                <div>
                  ...
                  <Button shape="circle" icon="delete" onClick={()=>this.deleteTitle(item)} n ghost/>
                </div>
              </div>
              {
  ...
  handleGenerate = (values) => {
   ...
   /* let forms=[];
    let i = this.state.forms.length+1;
    let title = {id:'t'+i, type: 'select', titleNumber: `第${i}题`, title: values.title,options:[]};

    for(let item in values){
      let str = /option/;
      if(str.test(item)){
        let option = {label: values[item],values: values[item]}
        title.options.push(option)
      }
    }

    forms.push(title);
    forms =  forms.concat(this.state.forms);
        ...
        >
          { security.title ? (
            <h3 className="header-title">
              ...
              <Icon onClick={this.handleAdd} type="plus-circle-o"  style={{ fontSize: 20}}/>
            </h3>
          ) : null}
