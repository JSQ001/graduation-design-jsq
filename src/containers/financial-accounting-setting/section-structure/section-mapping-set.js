...
class SectionMappingSet extends React.Component{
  constructor(props) {
    ...
    this.state = {
      ...
      columns:[
        {          /*科目段值代码*/
          title: formatMessage({id:"section.value.code"}), key: "sectionValueCode", dataIndex: 'sectionValueCode',
          render: (text, record, index) => this.renderColumns(text, record,index)
        },
        {          /*总账科目段值代码*/
          title: formatMessage({id:"section.value.code.total"}), key: "sectionValueCode1", dataIndex: 'sectionValueCode1',
          render: (text, record, index) => this.renderColumns(text, record,index)
        },
        {          /*删除*/
...
export default connect(mapStateToProps)(injectIntl(WrappedSectionMappingSet));
